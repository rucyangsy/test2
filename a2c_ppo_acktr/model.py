...
class NNBase(nn.Module):
    ...
    def _forward_gru(self, x, hxs, masks):  #出现gru再看
...
class CNNBase(NNBase):
    def __init__(self, num_inputs, recurrent=False, hidden_size=512,
        ...
        init_ = lambda m: init(m, nn.init.orthogonal_, lambda x: nn.init.
                               constant_(x, 0), nn.init.calculate_gain('relu'))  #正交矩阵初始化，常数初始化，返回推荐增益值

        if self.feat_from_selfsup_attention or self.feat_add_selfsup_attention:   #待看
            ...
            self._keypointer = Keypointer(in_channels=32, num_keypoints=self.selfsup_attention_num_keypoints,
                                          ...
                                          class_agnostic=self.selfsup_attention_keyp_cls_agnostic) #32， 7， 0.1， model, true
    ...
    def forward(self, inputs, rnn_hxs, masks, output_mask=False,
        ...
        if self.feat_from_selfsup_attention or self.feat_add_selfsup_attention:
            ...
            if self.block_selfsup_attention_grad:   #使用注意力时为false
                feat = feat.detach()
                keypoints_maps = keypoints_maps.detach()
                if keypoints_centers is not None:
                    keypoints_centers = keypoints_centers.detach()
            if self.selfsup_attention_keyp_maps_pool:   #使用注意力时为true
                keypoints_maps = keypoints_maps.max(dim=1, keepdim=True)[0] #[0]代表返回值，[1]代表索引
            meta['keypoints_maps'] = keypoints_maps

        if not self.feat_from_selfsup_attention:  #True
            x = self.convs_1(inputs_normalized)
            x = self.convs_2(x)
            x = self.convs_3(x)

            if self.feat_add_selfsup_attention:     #True
                if self.feat_mul_selfsup_attention_mask:    #True
                    if self.feat_mul_selfsup_attention_mask_residual:   #False
    ...
    def _train_selfsup_attention(self, input, target, config):
        input = input / 255.
        target = target / 255.
        autoencoder_loss = config.AUTOENCODER_LOSS     #True
        heatmap_sparsity_loss = config.HEATMAP_SPARSITY_LOSS   #True
...
class FeatureEncoder(nn.Module):
    def __init__(self, num_inputs, use_layer_norm=True,
                 use_instance_norm=False):                  #1, true, false
...
class Keypointer(nn.Module):
    ...
    def __init__(self, in_channels, num_keypoints, gauss_std, keypoint_encoder, class_agnostic=False):
        super(Keypointer, self).__init__()              #32, 7, 0.1, model, true
...
class SelfSupAttention(nn.Module):
    def __init__(self, feature_encoder, keypointer, decoder):
        ...
        self.avgpool = nn.AdaptiveAvgPool2d(1)   #自适应池化，变成1*1
    ...
    def forward(self, image_a, image_b, output_heatmaps=False, output_keypoints=False,
                ...
                output_keypoints_loc=False):   #output_heatmaps = True,autoencoder_loss=true
        ...
        num_keypoints = image_a_keypoints_maps.shape[1] #7
