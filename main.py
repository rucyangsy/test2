...
def main():
    ...
    for j in range(num_updates):
        ...
        rollouts.compute_returns(next_value, config.use_gae, config.gamma,
                                 config.gae_lambda, config.use_proper_time_limits) #use_gae = false, 0.99, 0.95, false

        value_loss, action_loss, dist_entropy = agent.update(rollouts)

        rollouts.after_update()

        if config.train_selfsup_attention and j > 15:    #自监督损失
            for _iter in range(config.num_steps // 5):
                frame_x, frame_y = rollouts.generate_pair_image(config.resized_size)  #[16,1,84,84]
        ...
        if j % config.log_interval == 0 and len(episode_rewards) > 1:   #记录信息
        ...
        if (config.eval_interval is not None and len(episode_rewards) > 1
                and j % config.eval_interval == 0):         #评估模型
            total_num_steps = (j + 1) * config.num_processes * config.num_steps
